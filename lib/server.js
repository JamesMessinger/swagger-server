 ...
 */
SwaggerServer.prototype.start = function start(port, options, callback) {
    var self = this;
    ...
    self.__ensureParsed(function(err, swagger) {
        ...
        if (self.__started) {
            return doCallback(callback, util.createError('Swagger-Server has already started. It cannot be started again.'));
        }

        if (self.__stopped) {
            return doCallback(callback, util.createError('The server has been stopped and cannot handle any further requests'));
        }
        ...
        if (scheme === 'http') {
            ...
            httpServer = https.createServer(options, self.middleware);
        }
        ...
        if (address && address.port) {
            ...
            self.state.url = url.parse(scheme + '://localhost:' + address.port);
        }
        else {
...
SwaggerServer.prototype.stop = function stop(callback) {
    ...
    if (!self.__stopped) {
        ...
        if (self.__httpServer) {
            self.__httpServer.removeListener('close', self.stop);
...
SwaggerServer.prototype.middleware = function middleware(req, res, next) {
    ...
    if (self.__stopped) {
        ...
        doCallback(next, util.createError('The server has been stopped and cannot handle any further requests'));
    }
...
SwaggerServer.prototype.__parse = function parse() {
    ...
    // If we're already in the middle of parsing, or the server has been stopped, then do nothing
    if (self.__parsing || self.__stopped) return;
...
SwaggerServer.prototype.__watchFiles = function watchFiles() {
    ...
    self.__watchedFiles = self.__watchedFiles || [];
    self.__checkForFileChange = function(curr, prev) {
    ...
    self.state.files.forEach(function(file) {
        fs.watchFile(file, {persistent: false, interval: 1000}, self.__checkForFileChange);
        self.__watchedFiles.push(file);
    });
};
...
SwaggerServer.prototype.__unwatchFiles = function unwatchFiles() {
    ...
    if (self.__watchedFiles) {
        self.__watchedFiles.forEach(function(file) {
            fs.unwatchFile(file, self.__checkForFileChange);
        });
        self.__watchedFiles = [];
    }
};
