(function() {
  ...
  function parseFile(value, schema) {
    ...
    // Parse the value
    var parsedValue = value;
    if (!(_.isObject(parsedValue) && parsedValue.path && parsedValue.size)) {
      throw syntaxError('Invalid or corrupted file')
    }
    ...
    if (schema.minLength) {
      ...
      if (isNaN(minLength)) {
        throw syntaxError('The "minLength" value specified in the Swagger spec is invalid (%s)', schema.minLength);
      }

      if (parsedValue.size < minLength) {
        throw syntaxError('File "%s" is only %d bytes. The minimum is %d bytes', parsedValue.originalname, parsedValue.size, minLength);
      }
    }
    ...
    if (schema.maxLength) {
      ...
      if (isNaN(maxLength)) {
        throw syntaxError('The "maxLength" value specified in the Swagger spec is invalid (%s)', schema.maxLength);
      }

      if (parsedValue.size > maxLength) {
        throw syntaxError('File "%s" is %d bytes. The maximum is %d bytes', parsedValue.originalname, parsedValue.size, maxLength);
      }
    }
