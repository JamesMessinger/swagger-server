(function() {
  ...
  module.exports = function postCollectionMock(req, res, next) {
    ...
    // Determine the resource's "ID"
    var resourceId = determineResourceId(newResource, req.swagger.operation.parameters);
   ...
   */
  function determineResourceId(resource, params) {
    var idName, idValue;
    ...
    if (params && params.length > 0) {
      // By default, use the "body" parameter's schema
      schema = _.result(_.findWhere(params, { in: 'body' }), 'schema');

      // If that fails, then use the first parameter
      if (!schema) {
    ...
    var propNames = Object.keys(resource);
    if (schema) {
      propNames = _.union(Object.keys(schema.properties), propNames);
    }
    ...
    // If no "ID" property was found, then fallback to the required properties
    if (!idName && schema && schema.required) {
    ...
    // Special case for file resources
    if (!idName) {
      var fileParams = _.where(params, { type: 'file', in: 'formData' });
      if (fileParams && fileParams.length === 1) {
        idValue = _.result(resource[fileParams[0].name], 'name');
      }
    }
