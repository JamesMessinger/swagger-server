    ...
    matchSchema: function matchResponseSchema(req, res, next) {
      ...
      _.each(res.swagger.successResponses, function(response, code) {
        if (response.schema) {
          ...
          schemaError = validateSchema.safeValidate(adjustedBody, response.schema);
          if (schemaError === undefined) {
            ...
            // There's a default value specified in the Swagger spec,
            // so we might use that (if we don't find any exact matches)
            defaultResponse = response.schema.default;
      ...
      if (!hasResponseSchema) {
        ...
        debug('According to the Swagger spec, this operation does not return any response content.');
        res.body = undefined;
      }
      else if (schemaError) {
        console.warn('WARNING! The response body does not match any of the response schemas for the "%s" %s operation.', req.path, req.method);

        if (defaultResponse) {
          console.warn('    The %s response has a default value defined in Swagger spec, so using that instead.', defaultResponseCode);
          res.body = defaultResponse;
        }
        else {
          console.warn(schemaError.stack || schemaError.message);
        }
      }
