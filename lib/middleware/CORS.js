    ...
    return function corsDefaults(req, res, next) {
      if (swaggerServer.settings.enableCORS) {
        ...
        if (swagger) {
          ...
          // Strip-off the basePath
          if (swagger.basePath && path.indexOf(swagger.basePath) === 0) {
            path = path.substr(swagger.basePath.length);
          }

          // Find this path in the spec
          if (swagger.paths && swagger.paths[path]) {
            path = swagger.paths[path];
            res.set('Access-Control-Allow-Methods', util.getAllowedMethods(path).join(', '));
            headerSet = true;
          }
        }
