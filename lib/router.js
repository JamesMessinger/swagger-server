(function() {
  ...
  function SwaggerRouter(swaggerServer) {
    ...
    this.middleware.use(function swaggerRouterFn(req, res, next) {
      ...
      var pathName = findSwaggerPath(reqPath);
      if (pathName) {
     ...
     * @param {string} path   the requested path (e.g. "/users/jdoe")
     * @returns {string}      the Swagger path name (e.g. "/users/{username}"), or undefined if not found
     */
    function findSwaggerPath(reqPath) {
      var swagger = swaggerServer.swaggerObject;
      var match;
      ...
      _.each(swagger.paths, function(pathObj, swaggerPath) {
      ...
      if (match) {
        debug('Matched request "%s" to Swagger path "%s"', reqPath, match);
      }
      else {
